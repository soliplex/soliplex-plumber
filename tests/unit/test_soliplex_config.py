"""Offline unit tests for the ``soliplex_plumber.soliplex_config`` core.

The module backs the ``soliplex-config`` console script. Tests are hermetic:
the ``docker``/``shutil.which`` and ``subprocess.run`` seams (the one-off
``soliplex-cli config`` container) are mocked and the filesystem is routed
through ``tmp_path`` -- no real Docker, no network.
...
# --------------------------------------------------------------------------
# run (console-script entry point)
# --------------------------------------------------------------------------
def test_run_returns_main_exit_code(monkeypatch, which, run, tmp_path, capsys):
    project = _make_project(tmp_path)
    run.return_value.stdout = _config_yaml("/environment/rooms/chat")
    monkeypatch.setattr(
        soliplex_config.sys,
        "argv",
        ["soliplex-config", "show", "--project-dir", str(project)],
    )

    rc = soliplex_config.run()

    assert rc == 0


def test_run_reports_soliplex_config_error(
    monkeypatch, which, run, tmp_path, capsys
):
    project = _make_project(tmp_path, compose=False)  # -> ComposeNotFound
    monkeypatch.setattr(
        soliplex_config.sys,
        "argv",
        ["soliplex-config", "show", "--project-dir", str(project)],
...
def test_run_reports_called_process_error(
    monkeypatch, which, run, tmp_path, capsys
):
    project = _make_project(tmp_path)
